        ...
        <div class="orders is-flex is-flex-direction-column is-justify-content-flex-start">
          ...
          is-align-items-center px-4">
              ...
              is-align-items-flex-start my-3">
                                    ...
                                    <span class="order-number my-3">{{__('Order number')}} : <span>{{
                  $order[0]->oeder_id }}</span></span>
                                    <span
                                        class="order-date my-3">{{__('Order created')}} : <span>{{ $order[0]->created_at  }}</span></span>
                                    <span class="order-status my-3">{{__('Status')}} : <span>{{ $order[0]->status ? 'Delivered' : 'In
                  process'
                  }}</span></span>
                                </div>
                ...
                is-align-items-flex-start my-3">
                                    ...
                                    <span
                                        class="customer-name my-3">{{__('Customer')}} : <span>{{ $order[0]->username }}</span></span>
                                    <span
                                        class="customer-email my-3">Email : <span>{{ $order[0]->email }}</span></span>
                                    <span
                                        class="customer-phone my-3">{{__('Phone number')}} : <span>{{ $order[0]->phone_number }}</span></span>
                                </div>
                            </div>
            ...
            is-align-items-flex-start px-3">
                                ...
                                @foreach($order as $product)
                                    <div class="product field is-flex   is-align-items-center my-3">
                                        <div class="cart-item__icon is-hidden-mobile">
                                            <img src="{{ asset('storage/uploads/images/'.$product->product_id
                      .'/700x700/' .
                                        ...
                                        <div
                                            class="product-quantity px-3">{{ $product->quantity}}</div>
                                        <div
                                            class="product-price  px-3">{{ $product->price/100}}</div>
                                        <div
                                            class="product-sum  px-3">{{ $product->quantity * $product->price/100}}</div>
                                    </div>
                                @endforeach
                            </div>
              ...
              is-align-items-stretch my-3">
                                    ...
                                    <div
                                        class="order-sum__without-delivery is-flex  is-justify-content-space-between my-3"><span
                                            class="has-text-weight-bold">{{__('Sum')}}:
                  </span> <span>$ {{ number_format($order[0]->sum/100 - 9.44,2,'.',',')}} </span>
                                    </div>
                                    <div
                                        class="delivery is-flex  is-justify-content-space-between my-3"><span
                                            class="has-text-weight-bold">{{__('Delivery')}}
                                                                                                        :</span>
                                        <span>$ 9.44</span></div>
                                    <div
                                        class="order-sum is-flex  is-justify-content-space-between my-3"><span
                                            class="has-text-weight-bold">{{__('Total')}}:
                    </span> <span>$ {{
                number_format($order[0]->sum/100,2,'.',',')}}
                  </span>
                                    </div>
                                </div>
                            </div>
