...
addToCartButtons.forEach((elem) => {
    ...
    elem.addEventListener("click", () => {
        ...
        if (
            ...
            document.cookie.match(/cart=[0-9,]+/g)
        ) {
            if (!checkCookie(productId)) {
                ...
                document.cookie =
                    ...
                    "," +
                    productId +
                    ";max-age=5000000;SameSite=Lax;path=/";
                changeAddToCartButton(elem);
                renderCartButton();
            }
        } else {
            document.cookie =
                "cart=" + productId + ";max-age=5000000;SameSite=Lax;path=/";
            changeAddToCartButton(elem);
            renderCartButton();
        }
    });
...
searchInput.addEventListener("keyup", (elem) => {
    ...
    function getSearchResult() {
        axios
            .get(
                "/api/search?search_string=" + encodeURIComponent(searchString)
            )
            .then((response) => {
                if (response.status === 200) {
...
function closeModals() {
    ...
    document
        .querySelector(".category-list__sorting")
...
let submitNewsSubscription = document.querySelector(
...
if (submitNewsSubscription) {
    submitNewsSubscription.addEventListener("click", (e) => {
        ...
        axios.get("/sanctum/csrf-cookie").then(() => {
            axios
                .post("/subscribe", { email: emailNewsSubscription.value })
                .then((res) => {
                    if (res.status === 200) {
                        ...
                        document.querySelector(
                            ".subscription"
                        ).innerHTML = `<h3 class="is-size-3 has-text-success">Subscribed!</h3>`;
                    }
                })
                .catch((err) => {
        ...
        el.addEventListener("click", (e) => {
            if (el.classList.contains("active-order-tab")) {
                ...
                el.parentElement.parentElement.style.height =
                    orderHeight + 60 + "px";
                el.querySelector(".category-filter__arrow").classList.add(
                    "reverse"
                );
            } else {
                el.classList.add("active-order-tab");
                el.parentElement.parentElement.style.height = 60 + "px";
                el.querySelector(".category-filter__arrow").classList.remove(
                    "reverse"
                );
            }
        });
